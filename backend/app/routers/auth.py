...
async def _create_verification(db: AsyncSession, email: str) -> str:
...
# mirrors the pattern already used for the public chatbot/webhook endpoints
# (routers/public.py) since anyone can call it with someone else's address.
_RESEND_RATE_LIMIT_WINDOW_SECONDS = 900
_RESEND_RATE_LIMIT_MAX_REQUESTS = 3
_resend_rate_limit_buckets: dict[str, list[float]] = {}


def _check_resend_rate_limit(email: str) -> bool:
    now = time.monotonic()
    bucket = _resend_rate_limit_buckets.setdefault(email, [])
    cutoff = now - _RESEND_RATE_LIMIT_WINDOW_SECONDS
    while bucket and bucket[0] < cutoff:
        bucket.pop(0)
    if len(bucket) >= _RESEND_RATE_LIMIT_MAX_REQUESTS:
        return False
    bucket.append(now)
    return True
...
async def resend_verification(body: dict = Body(...), db: AsyncSession = Depends(get_db)):
    ...
    if not _check_resend_rate_limit(email):
        return {"status": "ok"}
    ...
    code = await _create_verification(db, email)
...
async def update_user_access(
    ...
    + generic._authorize) specifically so this endpoint's checks can't be
    bypassed by going through the CRUD router instead.
    """
