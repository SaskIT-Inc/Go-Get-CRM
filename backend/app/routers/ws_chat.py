...
Communication (restricted client chat). A browser WebSocket handshake can't
carry a custom Authorization header, so the access token travels as a query
param instead — same JWT `apiClient.js` already sends as a Bearer header on
every REST call, just relocated for this one connection.

This endpoint only ever *receives* to detect disconnects; all real payloads
are server-initiated pushes from ws_manager.push(), called from generic.py's
create_entity right after a Message/Communication commit.
"""
